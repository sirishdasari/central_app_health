...
class _GuitarPracticeHomeScreenState extends State<GuitarPracticeHomeScreen> {
  ...
  Widget _card(GuitarPractice p) => Container(margin: const EdgeInsets.only(bottom: 12), padding: const EdgeInsets.fromLTRB(10, 10, 4, 10), decoration: BoxDecoration(color: card, borderRadius: BorderRadius.circular(20), border: Border.all(color: border)), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [IconButton(onPressed: () => _toggle(p), icon: Icon(p.completed ? Icons.check_circle : Icons.radio_button_unchecked, color: p.completed ? green : Colors.white54, size: 28)), const SizedBox(width: 2), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(p.title, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.w800)), const SizedBox(height: 6), Wrap(spacing: 6, runSpacing: 5, children: [_chip(p.category), _chip(p.level, purple: true)]), const SizedBox(height: 5), Row(children: [const Icon(Icons.schedule_rounded, color: muted, size: 15), const SizedBox(width: 5), Text(p.suggestedTime.isEmpty ? 'No suggested time' : p.suggestedTime, style: const TextStyle(color: muted)), const SizedBox(width: 10), const Icon(Icons.timer_outlined, color: muted, size: 15), const SizedBox(width: 4), Text('Practiced ${_time(p.dailyPracticeTime)}', style: const TextStyle(color: muted))]), if (p.description.trim().isNotEmpty) ...[const SizedBox(height: 4), Text(p.description, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: muted, fontSize: 13))], if (p.link.trim().isNotEmpty) TextButton.icon(onPressed: () => _openLink(p.link), style: TextButton.styleFrom(padding: EdgeInsets.zero, minimumSize: const Size(0, 30), tapTargetSize: MaterialTapTargetSize.shrinkWrap), icon: const Icon(Icons.link_rounded, color: purple, size: 16), label: const Text('Open practice video', style: TextStyle(color: purple, fontWeight: FontWeight.w700)))]), Column(children: [if (!p.completed) IconButton(tooltip: 'Start practice', onPressed: () => _start(p), style: IconButton.styleFrom(backgroundColor: green, foregroundColor: bg), icon: const Icon(Icons.play_arrow_rounded, size: 28)), PopupMenuButton<String>(onSelected: (v) { if (v == 'edit') _edit(p); if (v == 'delete') _delete(p); }, itemBuilder: (_) => const [PopupMenuItem(value: 'edit', child: Text('Edit')), PopupMenuItem(value: 'delete', child: Text('Delete'))])])],));
