...
class _GuitarPracticeHomeScreenState extends State<GuitarPracticeHomeScreen> {
  ...
  Map<String, dynamic> _payload() {
    ...
    return {
      ...
      'practices': (current?.practices ?? const <GuitarPractice>[]).map((p) {
        return {
          'id': p.id,
          'title': p.title,
          ...
          'dailyPracticeSeconds': p.dailyPracticeSeconds,
  ...
  Future<void> _toggle(GuitarPractice practice) async {
    try {
      await api.update(practice.id, {'completed': !practice.completed});
      await _load();
    } catch (e) {
  ...
  int get _todaySeconds {
    ...
    return (data?.practices ?? const <GuitarPractice>[]).fold<int>(0, (sum, p) {
      final date = DateTime.tryParse(p.practiceDate);
      if (date == null ||
          date.year != now.year ||
  ...
  int get _streak {
    ...
    for (final practice in data?.practices ?? const <GuitarPractice>[]) {
      if (practice.dailyPracticeSeconds <= 0) continue;
      final date = DateTime.tryParse(practice.practiceDate);
      if (date != null) {
  ...
  Widget build(BuildContext context) {
    ...
    final progress = target <= 0
        ? 0.0
        : (today / (target * 60)).clamp(0.0, 1.0).toDouble();
  ...
  Widget _dashboard(int today, int target, double progress) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final streakBox = _box(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Icon(Icons.local_fire_department_rounded, color: green, size: 28),
              const SizedBox(height: 8),
              const Text('Streak', style: TextStyle(color: muted, fontSize: 13)),
              Text(
                '$_streak days',
                style: const TextStyle(
                  color: Colors.white,
              ...
              Text(
                _streak == 0 ? 'Start today!' : 'Keep going!',
            ...
            if (narrow)
              Column(
                children: [
                  ...
                  SizedBox(width: double.infinity, child: streakBox),
                ],
              )
            else
              Row(
                ...
                children: [
                  ...
                  Expanded(flex: 3, child: streakBox),
                ],
              ),
            const SizedBox(height: 10),
            Container(
              ...
              children: [
                ...
                if (practice.link.trim().isNotEmpty)
                  TextButton.icon(
                    ...
                    icon: const Icon(Icons.link_rounded, color: purple, size: 16),
                    label: const Text(
                      'Open practice video',
  ...
  void _save() {
    ...
    if (titleValue.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Enter a practice title')),
      );
      return;
    }
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              TextField(
                controller: title,
                decoration: _dec('Title', Icons.music_note),
                style: const TextStyle(color: Colors.white),
              ),
              const SizedBox(height: 10),
              TextField(
                controller: time,
                decoration: _dec('Suggested time (HH:mm)', Icons.schedule),
                style: const TextStyle(color: Colors.white),
              ),
              const SizedBox(height: 10),
              TextField(
                ...
                decoration: _dec('Duration (minutes)', Icons.timer),
                style: const TextStyle(color: Colors.white),
              ),
              const SizedBox(height: 10),
              TextField(
              ...
              TextField(
                ...
                decoration: _dec('Practice link', Icons.link_rounded),
                style: const TextStyle(color: Colors.white),
              ),
